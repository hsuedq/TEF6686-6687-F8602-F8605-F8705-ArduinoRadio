/*
  ...
  changes:
  Sketch compatible with NXP-TEF668x (F8602 or F8605) 
...
static const uint8_t DSP_INIT[] PROGMEM =
{
  ...
  0x17, 0x1B, 0x63, 0x69, 0x64, 0x3B, 0x66, 0x09, 0x66, 0xE7, 0x67, 0x0B, 0x67, 0x1A, 0x67, 0x29, 0x68, 0x87, 0x68, 0x99, 0x68, 0xA7, 0x68, 0xB2, //End of LutByteValues & Patch 2.24
  0x03, 0x1C, 0x00, 0x00,                                           // Clear Required Initialization Control
        ...
        case 'M':
         ...
         case 'T':  // Tune to a frequency (to do keep filter settings)
          ...
          if ((REG_FREQ >= 65000) && (REG_FREQ <= 108000)) {
            ...
            if (radio_mode == 0) {                         // from AM to FM then switch to adaptive filter AUTO in TEF-GTK
              if (Filter_AM != 16) {Filter_AM = current_filter;} 
             if (Filter_FM == 16) {Filter_FM = -1;}
              current_filter = Filter_FM;
              Set_Cmd(32, 10, 4, current_filter == -1 ? 1 : 0, pgm_read_word_near(FMFilterMap + current_filter),  1000, 1000);
